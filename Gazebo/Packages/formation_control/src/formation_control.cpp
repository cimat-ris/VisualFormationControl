...
and process some vision-based formation_control control.
*/
...
int main(int argc, char **argv){
	...
	ros::Publisher g = nh.advertise<formation_control::gamma>("/hummingbird"+me_str+"/gamma", 1);
	ros::Publisher image_descriptor_publisher = nh.advertise<formation_control::image_description>("/hummingbird"+me_str+"/image_description", 1);
	...
	for(int i=0;i<ns;i++){
		...
		ros::Subscriber n_s = nh.subscribe<formation_control::image_description>("/hummingbird"+j+"/image_description",1,&Agent::getImageDescription, &this_drone);
		subs_neighbors.push_back(n_s);
		ros::Publisher gm_p = nh.advertise<formation_control::geometric_constraint>("/hummingbird"+me_str+"/geometric_constraint"+j,1);
	...
	for(int i=0;i<nr;i++){
		...
		ros::Subscriber n_s = nh.subscribe<formation_control::geometric_constraint>("/hummingbird"+j+"/geometric_constraint" +me_str, 1,&Agent::getGeometricConstraint, &this_drone);
		subs_neighbors.push_back(n_s);
	}
	...
	if(gamma_file == 0 && needsAltitudeConsensus(controller_type)==1){
		for(int i=0;i<n;i++){
			if(i!=this_drone.getLabel()){
				...
				ros::Subscriber l = nh.subscribe<formation_control::gamma>("/hummingbird"+j+"/gamma",1,&Agent::setGamma, &this_drone);
