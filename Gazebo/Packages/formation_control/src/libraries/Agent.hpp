...
#include <formation_control/gamma.h>
...
class Agent{
	...
	public: 
		...
		formation_control::geometric_constraint sendGeometricConstraintMsg(int index);//send the corresponding geometric constraint
		...
		formation_control::image_description getImageDescription(); //to send the msg with image description
		trajectory_msgs::MultiDOFJointTrajectory move(double dt); //to calculate the new pose of the agent
		void getImageDescription(const formation_control::image_description::ConstPtr& msg);//computes constraint and velocities from msg		
		void getGeometricConstraint(const formation_control::geometric_constraint::ConstPtr& msg); //obtains geometric constraint from message and process it
		formation_control::gamma getGamma();//get the value gamma for this agent, ready to send as message
		void setGamma(const formation_control::gamma::ConstPtr& msg);//sets the gamma value for the agent specified in the message
