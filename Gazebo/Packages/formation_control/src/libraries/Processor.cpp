...
*/
formation_control::image_description Processor::getImageDescription(){
	return id;
}
...
*/ 
formation_control::geometric_constraint Processor::getGM(int index){
	return gm[index];
}
...
int *Processor::BRCommunicationSends(int label, int n_neigh, int *neighbors, int *n_s){	
	...
	for (int i=0;i<n_neigh;i++)
		if(label < neighbors[i]){
			...
			formation_control::geometric_constraint g;
			gm.push_back(g);
...
/* 
	...
	params: 
		...
		t: posible relative translation vector (int a vector of size 3) obtained from 
			...
			consensus
*/
Mat Processor::getGeometricConstraint(const formation_control::image_description::ConstPtr& msg, int *you, double *pose_i, double *pose_j, int *SUCCESS, int *n_matches, double *R, double *t){	
	...
	*n_matches = goodMatches.size();
	...
	if(isHomographyConsensus(controller_type) && goodMatches.size() > 4){
		vector<int> mask;
		GM = findHomography(p2, p1 ,RANSAC, 1,mask);
		...
		Mat mask;
		GM = findEssentialMat(p2, p1, K, RANSAC, 0.999, 1.0, mask); //compute essential matrix
		
		if(!DONE[*you]){
			Mat Rotation, translation;			
			recoverPose(GM, p2, p1, K, Rotation, translation, mask);//decompose using the mask
			for(int i=0;i<3;i++){
		...
		Mat mask; double x_mid = K.at<double>(0,2),y_mid = K.at<double>(1,2); int nn = p1.size(); 
		for(int i=0;i<nn;i++){
			p1[i].x-=x_mid;
			p2[i].x-=x_mid;
			p1[i].y = y_mid -p1[i].y;
		...
		GM = findFundamentalMat(p1,p2,FM_RANSAC,1.0,0.99,mask);
	}
