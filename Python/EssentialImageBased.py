...
"""
from Functions.Formation import line_formation, circle_formation, get_L_radius, get_bearings, get_A
from Functions.Initialize import verified_random_restricted, copy_cameras, order_cameras, find_nearest
from Functions.Control import IBEC
...
# Error parameters
threshold_p = 0.05 #threshold for translation error
threshold_w = 0.05 #threshold for rotation error
...
prim =0
#=================================================================init algorithm
while (e_t > threshold_p or e_psi > threshold_w) and ite < max_ite and rank >= 4*n_cameras-5:
	g = A.dot(g)
	if e_psi > threshold_w and done==False:
		p, R, vi, wi,e_r = IBEC(n_cameras,init_cameras,w_points,n_points,R_aster,p_aster,lambdaw,lambdav,L,{},{},False)
	elif e_t > threshold_p:
		done = True
		p, R, vi, wi,e_r = IBEC(n_cameras,init_cameras,w_points,n_points,R_aster,p_aster,lambdaw,lambdav,L,{},{},True)
...
plot_all('Image-based formation control using Essential Matrix',xx,yy,zz,n_cameras,x,y,z,copy,init_cameras,desired_cameras,init_poses,desired_poses,ite,t_arr,err_s,[],v,w,err_t,err_psi,dists)
run_info('Image-based formation control using Essential Matrix',seed_r,seed_p,ite,dt,threshold_p,n_cameras,rank,sixth,e_t,e_psi,sd,L,p_aster,R_aster,copy_poses,init_poses,e_s)
